...
export async function fetchVideoTranscript(videoId, options = {}) {
  ...
  if (lang === "en") {
    try {
      ...
      const sample = built.slice(0, 6).map((item) => item.text).join(" ");
      sourceLang = await detectLang(sample);

      if (sourceLang && sourceLang !== "auto" && !sourceLang.startsWith("en")) {
        const translated = await translateSegmentsToEnglish(built);
        rawSegments = translated.segments.map((item) => ({
          text: item.text,
          offset: item.offset,
        }));
        translatedByAI = true;
  ...
  if (translatedByAI && segments.length) {
